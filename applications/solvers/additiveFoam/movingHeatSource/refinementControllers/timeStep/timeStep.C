...
Foam::refinementControllers::timeStep::timeStep
(
...
bool Foam::refinementControllers::timeStep::update()
{
    //- Refine in regions above specified temperature
    refinementController::refineUsingTemperature();
    
    //- Refine current beam position
    refinementController::refineUsingTime(mesh_.time().value() + 2 * SMALL);

    return true;
}


bool Foam::refinementControllers::timeStep::read()
{
